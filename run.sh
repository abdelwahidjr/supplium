...
DB_HOST="mysql"
DB_PORT="3306"
DB_DATABASE="supplium_db"
DB_USERNAME="root"
DB_PASSWORD="root"

db_permission="chown mysql:mysql /etc/mysql/conf.d/my.cnf && chmod 0444 /etc/mysql/conf.d/my.cnf"
recreate_db="mysql -u$DB_USERNAME -p$DB_PASSWORD -P$DB_PORT -h$DB_HOST --execute='DROP DATABASE IF EXISTS $DB_DATABASE;CREATE DATABASE $DB_DATABASE;'"

##################################################
# docker
containers="nginx php-fpm mysql phpmyadmin redis workspace"
...
#==========
build_project() {
echo "$msg_conn && sleep 2 && $d exec mysql bash -c \"$db_permission && $recreate_db\" && $msg_db_recreated && $msg_install && $(os_commands "workspace_bash") -c \"composer install\" && $(laravel_seed)  && $(laravel_permission)"
}
...
# ==========
##################################################################################################################
- "project start"                                "$c && $d up -d $containers && $msg_docker_build && $msg_container_start"
...
- "docker restart"                               "./run.sh docker stop && ./run.sh docker up"
- "docker stop"                                  "$c && $d stop && $msg_docker_stop"
- "docker down"                                  "$c && $d down && $msg_docker_down"
- "docker rebuild images"                        "./run.sh docker down && $msg_container_build && $c && $d up --build -d $containers && $msg_docker_build"
- "docker build images without cache"            "./run.sh docker down && $msg_container_build && $c && $d build --no-cache $containers && $d up --build -d $containers && $msg_docker_build"
...
- "remove docker data directory"                 "$c && sudo rm -rf ./docker-data/ && $msg_docker_data "
- "cancel"                                       "$msg_cancel"
